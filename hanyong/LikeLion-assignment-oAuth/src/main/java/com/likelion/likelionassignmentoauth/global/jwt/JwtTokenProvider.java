package com.likelion.likelionassignmentoauth.global.jwt;
...
public class JwtTokenProvider {
    ...
    private static final String AUTHORITES_KEY = "auth"; //권한 키 상수 추가
    ...
    public String generateToken(Member member) {
        ...
        return Jwts.builder()
                ...
                .claim(AUTHORITES_KEY, member.getRole().toString()) //추가된 권한 Claim
                .issuedAt(now)  // 발행 시간
    ...
    public Authentication getAuthentication(String token) {
        ...
        String authority = claims.get(AUTHORITES_KEY, String.class);
        if (authority == null) {
            throw new BusinessException(ErrorCode.NO_AUTHORIZATION_EXCEPTION, "권한 정보가 없는 토큰입니다.");
        }
        List<GrantedAuthority> authorites = Arrays.stream(authority.split(","))
                .map(SimpleGrantedAuthority::new)
                .collect(Collectors.toList());
        //memberId, 공백, authorites 반환
        return new UsernamePasswordAuthenticationToken(claims.getSubject(), "", authorites);
    }
